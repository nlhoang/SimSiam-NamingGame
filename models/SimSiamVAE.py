...
def loss_fn(p1, p2, z1, z2, mu1, logvar1, mu2, logvar2, loss_type='cosine', loss_rec=True, beta=0.0001):
    ...
    total_loss = loss_simsiam + loss_recon + beta * loss_kl
    return total_loss, loss_simsiam, loss_recon, beta * loss_kl


def train(model, dataloader, learning_rate, device, epochs=100, save_interval=100, save_prefix='SimSiamVAE_CIFAR'):
    ...
    for epoch in range(epochs):
        ...
        for batch_idx, (x1, x2) in enumerate(dataloader):
            ...
            loss, loss_simsiam, loss_recon, loss_kl = loss_fn(p1, p2, z1, z2, mu1, logvar1, mu2, logvar2)
            train_loss += loss.item()
            recon_loss += loss_recon.item()
            simsiam_loss += loss_simsiam.item()
            kl_loss += loss_kl.item()
            loss.backward()
            optimizer.step()

        avg_loss = train_loss / D
        avg_recon_loss = recon_loss / D
        avg_simsiam_loss = simsiam_loss / D
        avg_kl_loss = kl_loss / D
        loss_history.append(avg_loss)
        print(f'====> Epoch {epoch}, Avg Loss: {avg_loss:.4f}, SimSiam Loss: {avg_simsiam_loss:.4f}, Recon Loss: {avg_recon_loss:.4f}, KL Loss: {avg_kl_loss:.4f}')
...
if __name__ == "__main__":
    ...
    # Training
    model = SimSiamVAE(feature_dim=512, latent_dim=2048, variable_dim=1024, backbone='cnn').to(device)
    print(model)
    print('Model Size: {}'.format(param_count(model)))
    loss_history = train(model, train_dataloader, learning_rate=0.001, device=device, epochs=200, save_interval=50, save_prefix='SimSiamVAE_CIFAR')
